...
| `cache` | the read-through store: SQLite (`pages`, `searches`, `distillations`, `snapshots`), FTS5, optional embeddings — with a **sqlite-vec vec0 ANN index** (`page_vectors`, embeddings builds) that leads semantic recall; the `embeddings` BLOB table stays the source of truth and the index lazily sizes itself to the first vector, self-heals from the BLOBs, and parks (→ brute-force cosine fallback) on any failure. First-hit lookup, conditional-GET refresh, write-back. Snapshots (raw HTML, TTL-pruned) are interaction working memory, never recalled. |
...
embeddings (url, vec BLOB)            -- bge-small, Micro+ only (source of truth)
page_vectors (vec0, float[dim])       -- sqlite-vec ANN index, derived + self-healing
